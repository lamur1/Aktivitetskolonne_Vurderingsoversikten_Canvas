...
// ============================================================
// NRK-video og H5P
// Laster inn støtte for NRK-videoavspilling og H5P-innhold
// ============================================================
...
// ============================================================
// Min fremdrift — modal med batterigrafikk og statistikk
//
// Viser elevens fremdrift gjennom kurset i en modal:
//   • Antall leksjoner med innleveringer
//   • Antall innleveringer som venter på tilbakemelding
//   • Snitt-prosent for sett lærestoff
//   • Batterigrafikk per leksjon (grønn = sett, prikker = innlevert/mangler)
//
// Aktiveres via flytende knapp nederst til høyre (skjules når
// leksjonsfremdriftsbjelken er synlig — pille i bjelken tar over).
// Vises bare for elever. Cache: localStorage, 1 time.
// ============================================================
...
(function () {
  ...
  function renderData(data) {
    ...
    // Snitt visning — gjennomsnitt av completed/total for leksjoner eleven har startet.
...
// ============================================================
// Leksjonsfremdrift — horisontal kuleramme per modul
//
// Viser en sticky bar under brødsmulestien med én kule per
// modulitem. Gjeldende item er markant større og mørk.
// Fullførte items: transparent grønn fyll + grønn kant.
// Øvrige: hvit fyll, svart kant.
// Linjen mellom kulene har grønn skygge der innholdet er fullført.
// Oppgaver/quiz markeres med avrundet firkant + amber-prikk.
//
// På kurssider uten leksjonskontekst vises en minimal bar med
// kun "Min fremdrift"-pille (og leksjonsstatus fra cache).
//
// Vises bare for elever. Cache: sessionStorage, 5 min.
// ============================================================
...
(function () {
  ...
  function render(mod, currentItemId) {
    ...
    const SH_GREEN_TOP   = 'rgba(52,160,71,0.55)';
  ...
  // ── Delte hjelpere: render() og renderMinimalBar() ────────────────────────
  ...
  function renderMinimalBar() {
    ...
    // Vis "X av Y leksjoner" fra cache om tilgjengelig
  ...
  // ─── To-do navigasjonshjelper ──────────────────────────────────────────────
  // Lagrer modul-item-ID i sessionStorage ved klikk på To-do-lenker slik at
  // kulerammen vises riktig selv om Canvas redirecter til /taking/ og stripper
  // module_item_id fra URL-en.
  document.addEventListener('click', function (e) {
    const link = e.target.closest(
      '.planner-todosidebar a[href*="/courses/"], .ToDoSidebar a[href*="/courses/"], ' +
      '.todo-list a[href*="/courses/"], [data-testid="todo-item"] a[href*="/courses/"], ' +
      '.PlannerItem a[href*="/courses/"]'
    );
    if (!link || link.href.includes('module_item_id')) return;
    try {
      ...
      if (!courseMatch || !contentMatch) return;
      const cId       = courseMatch[1];
      const contentId = contentMatch[2];
      const cacheKey  = `ef_chevron_${cId}_${userId}`;
      const cached    = JSON.parse(sessionStorage.getItem(cacheKey) || 'null');
      if (!cached || !cached.data) return;
      for (const mod of cached.data) {
        for (const item of (mod.items || [])) {
          if (String(item.content_id) === String(contentId)) {
            sessionStorage.setItem('gs_todo_nav', JSON.stringify({
              courseId: cId, itemId: String(item.id), contentId
            }));
            return;
          }
        }
...
(function () {
  ...
  var observer = new MutationObserver(injiserElementnummer);
