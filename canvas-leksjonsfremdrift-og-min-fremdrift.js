...
(function () {
  ...
  async function fetchAllData() {
    ...
    const modules = modulesRaw.map(mod => {
      ...
      const withReq  = items.filter(it => it.completion_requirement);
  ...
  function renderData(data) {
    ...
    contentEl.innerHTML = `
      ...
      ${venter > 0 ? `<div style="margin-top:6px;font-size:12px;color:#5f5e5a;">
...
(function () {
  ...
  function render(mod, currentItemId) {
    ...
    items.forEach((item, i) => {
      ...
      bead.addEventListener('mouseenter', () => {
        bead.style.transform = 'scale(1.16)';
        bead.style.zIndex = '2';
      });
      bead.addEventListener('mouseleave', () => {
        bead.style.transform = '';
        bead.style.zIndex = '1';
        bead.style.boxShadow = beadShadow;
      });
  ...
  function makePill() {
    ...
    pill.innerHTML = `<svg width="11" height="11" viewBox="0 0 17 17" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" aria-hidden="true">
      <rect x="1.5" y="3.5" width="14" height="10" rx="2"/>
      ...
      try {
        ...
        if (raw) {
          ...
          if (Date.now() - ts < 60 * 60 * 1000 && data && data.totalLeksjoner) {
            ...
            summary.textContent = `${data.godkjent} av ${data.totalLeksjoner} leksjoner med innleveringer`;
    ...
    for (const mod of modules) {
      for (const item of (mod.items || [])) {
        ...
        try {
          ...
          if (itemPath.replace(/\/$/, '') === pathname.replace(/\/$/, '')) {
            return { mod, item };
          }
